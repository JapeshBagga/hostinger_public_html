...
if($row['role']=='hod'){
	...
	$atd_query="select atd_date from attendance where (atd_type='faculty' and user_id=$user_id and atd_date='$date')";
	$atd_result=mysqli_query($conn, $atd_query);
	$atd_row=mysqli_fetch_assoc($atd_result);
	$atd_rowcount=mysqli_num_rows($atd_result);
	
	if($atd_rowcount <= 0){
		
		$atd_faculty_name = $first_name." ".$last_name;
		$atd_subject = "C";
		$atd_dept ="IT";
		$atd_present_days="1";
		$atd_absent_days="0";
		...
		$query = "insert into attendance (atd_type,atd_faculty_name, atd_dept, atd_subject, atd_present_days, atd_absent_days, atd_total_days, atd_date, user_id) 
		values ('$atd_type','$atd_faculty_name','$atd_dept','$atd_subject','$atd_present_days','$atd_absent_days','$atd_total_days','$date','$user_id' )";
		$atd_result=mysqli_query($conn, $query);
	}
}
?> 
<!doctype html>
...
<head>
    ...
    <title>Dashboard | BonFinitics - Makose</title>
