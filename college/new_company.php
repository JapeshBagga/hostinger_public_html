                    ...
                    <div class="app-main__inner">
                        ...
                        <div class="">
                            ...
                            <?php                                
                                ...
                                $query = "SELECT * FROM CollegeCompanyDashboard.Companies WHERE id NOT IN (SELECT to_id FROM CollegeCompanyDashboard.Requests WHERE flag = 0 AND from_id=$collegeID) AND id NOT IN (SELECT from_id FROM CollegeCompanyDashboard.Requests WHERE flag = 1 AND to_id=$collegeID) AND id NOT IN (SELECT compId FROM CollegeCompanyDashboard.accepted WHERE collId=$collegeID) ORDER BY name;";
                                // $query->execute();
                            ...
                            <?php
                                if(isset($_POST['connect']))
                                {
                                    ...
                                    $companyID = (int)$_POST['connect'];
                                    $query = $conn->prepare("INSERT INTO Requests (from_id, to_id, flag) VALUES (?,?,?)"); 
                                    $flag = 0;
                                    $query->bind_param("iii", $collegeID, $companyID,$flag);
                                    $query->execute();
